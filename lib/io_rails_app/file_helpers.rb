...
module FileHelpers
  ...
  def self.replace_string(key,new_value,original_file,target_file = nil)
    ...
    p "Something went wrong. The file '#{target_file}' couldn't be created and/or updated. Aborting app creation.", true
    abort
  ...
  def self.override_file(source_file, target_file)
    ...
    p "Couldn't override #{target_file}. Aborting app creation.", true
    abort
  end
  ...
  def self.add_to_file(target_file, text)
    ...
    p "Couldn't write inside #{target_file}. Aborting app creation.", true
    abort
  end
